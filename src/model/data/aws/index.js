...
async function deleteFragment(ownerId, id) {
  ...
  // Create the DELETE params for DynamoDB
  const ddbParams = {
    TableName: process.env.AWS_DYNAMODB_TABLE_NAME,
    Key: {
      ownerId,
  ...
  // Create the DELETE commands to send to S3 and ddb
  const command = new DeleteObjectCommand(s3Params);
  const ddbDeleteCommand = new DeleteCommand(ddbParams);

  try {
    // Use our client to send the command and delete the object
    await s3Client.send(command);
